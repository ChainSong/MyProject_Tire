...
@Component
export default class ReceiptEdit extends AbpBase {
  ...
  get() {
    this.$store
      ...
      .then((res) => {
        this.header = res;
        this.details.line = res.receiptDetails;
      })
      .catch((err) => {
        console.log(err);
      });
  ...
  visibleChange(value: boolean) {
    if (!value) {
      ...
      this.header = Util.extend(
        true,
        {},
        this.$store.state.receipt.editReceipt
      );
      this.get();
    }
  }
        ...
        (this.$refs.details as any).validate(async (valid: boolean) => {
          if (valid) {
            this.$store.dispatch({
              ...
              data: {
                Receipt: this.header,
                ReceiptDetails: this.details.line,
              },
            });
